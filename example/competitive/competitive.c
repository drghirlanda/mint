...
/* sets an peaked activity profile over the 8 input units, something
...
int main( void ) {
  ...
  int t1, t2, i, size, x;   /* various counters and helpers */   
  ...
  for( t1=0; t1<64; t1++ ) { /* loop over 64 stimulus presentations */
    ...
    for( t2=0; t2<50; t2++ ) {
      ...
      mint_network_nupdate( net ); /* update node activity */
      mint_network_wupdate( net ); /* update weight values */
